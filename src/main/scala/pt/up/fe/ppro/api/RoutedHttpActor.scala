...
object RoutedHttpActor {
  def props(route: Route) = Props(classOf[RoutedHttpActor], route)
}
 ...
 */
class RoutedHttpActor(val route: Route) extends HttpServiceActor with ActorLogging {
